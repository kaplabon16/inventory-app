...
export default function InventoryPage() {
  ...
  const idPrev = useMemo(() => renderIdPreview(elements), [elements]);

  if (!inv) return <div className="p-6">Loading…</div>;

  const itemCols = [
    { key: 'customId', title: 'ID',
      render: (v, r) => <Link to={`/inventories/${id}/items/${r.id}`} className="text-blue-600">{v}</Link> },
  ...
  return (
    <div className="max-w-6xl p-4 mx-auto">
      <div className="flex items-center gap-3">
        ...
        <button onClick={saveInventory} className="px-2 py-1 text-sm border rounded">{t('save')}</button>
      </div>
      ...
      {tab==='customId' && (
        <div className="grid gap-3 mt-3">
          <div><b>{t('preview')}:</b> <code className="px-2 py-1 bg-gray-100 rounded dark:bg-gray-800">{idPrev}</code></div>
          ...
          {elements.sort((a,b)=>a.order-b.order).map((el,idx)=>(
            <div key={idx} className="grid items-center gap-2 md:grid-cols-4">
              ...
              <button onClick={()=>{ const next=[...elements]; next.splice(idx,1); setElements(next); }}
          ...
          <div>
            ...
            <button onClick={async()=>{ await api.post(`/api/inventories/${id}/custom-id`, { elements }); await load(); }}
                ...
                <div key={idx} className="grid items-center gap-2 mb-2 md:grid-cols-4">
                  ...
                  <label className="flex items-center gap-2">
                    <input type="checkbox" checked={!!f.show}
                      onChange={e=>{ const next={...fields}; next[group][idx].show=e.target.checked; setFields(next); }}/>
          ...
          <div key={x.userId} className="flex items-center justify-between px-3 py-2 border-b">
            ...
            <div className="flex gap-2">
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={!!x.canWrite}
                  onChange={async(e)=>{ await api.put(`/api/inventories/${id}/access/${x.userId}`, { canWrite: e.target.checked }); await load(); }}/>
                write
              </label>
              <button className="px-2 py-1 text-sm border rounded"
...
function StatsTab({ id }) {
  ...
  useEffect(()=>{ (async()=>{ const res = await api.get(`/api/inventories/${id}/stats`); setData(res.data); })(); },[id]);
