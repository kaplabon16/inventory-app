...
router.get('/public-recent', async (req, res) => {
  ...
  res.json(list.map(x => ({
    ...
    imageUrl: x.image1 || x.image2 || x.image3 || null,
...
router.put('/:id', requireAuth, async (req, res) => {
  ...
  const { version, title, description, publicWrite, categoryId, images } = req.body || {}
  ...
  if (Array.isArray(images)) {
    data.image1 = images[0] || null
    data.image2 = images[1] || null
    data.image3 = images[2] || null
  }
...
router.post('/:id/fields', requireAuth, async (req, res) => {
  ...
  const orderMap = {}
  order.forEach((x, idx) => { orderMap[`${x.group}-${x.slot}`] = idx })
...
/* ---------- ITEMS ---------- */
async function invWithAccess(id) {
...
router.post('/:id/items', requireAuth, async (req, res) => {
  const { inv, access } = await invWithAccess(req.params.id)
  if (!inv) return res.status(404).json({ error: 'Not found' })
...
router.get('/:id/items/:itemId', requireAuth, async (req, res) => {
  const { inv, access } = await invWithAccess(req.params.id)
  if (!inv) return res.status(404).json({ error: 'Not found' })
  const item = await prisma.item.findUnique({ where: { id: req.params.itemId }, include: { _count: { select: { likes: true } } } })
...
router.post('/:id/items/:itemId/like', requireAuth, async (req, res) => {
  const { inv } = await invWithAccess(req.params.id)
  if (!inv) return res.status(404).json({ error: 'Not found' })
  ...
  else await prisma.like.create({ data: { itemId: item.id, userId: req.user.id } })
