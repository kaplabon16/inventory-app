...
router.get('/:id/access', requireAuth, async (req, res) => {
  ...
  res.json(rows.map(r => ({ userId: r.userId, name: r.user.name, email: r.user.email, canWrite: r.canWrite })))
})
...
// Read item (auth: edit screen shows private data)
router.get('/:id/items/:itemId', requireAuth, async (req, res) => {
  ...
  let liked = false
  if (req.user?.id) {
    const lk = await prisma.like.findUnique({
...
/* --------------------------------
 * LIKES (toggle; any authenticated user)
 * -------------------------------- */

// Count + did-I-like
router.get('/:id/items/:itemId/likes', optionalAuth, async (req, res) => {
  const item = await prisma.item.findUnique({ where: { id: req.params.itemId } })
  if (!item || item.inventoryId !== req.params.id) return res.status(404).json({ error: 'Not found' })
  const [count, mine] = await Promise.all([
    prisma.like.count({ where: { itemId: item.id } }),
    req.user?.id ? prisma.like.findUnique({
      where: { itemId_userId: { itemId: item.id, userId: req.user.id } }
    }) : null
  ])
  res.json({ count, liked: !!mine })
})
...
router.get('/:id/stats', async (req, res) => {
  ...
  // numeric titles from schema
  const numFields = await prisma.inventoryField.findMany({
    where: { inventoryId: id, type: 'NUMBER' }
  })
  const titles = { 1: (numFields.find(f=>f.slot===1)?.title || 'Number 1'),
                   2: (numFields.find(f=>f.slot===2)?.title || 'Number 2'),
                   3: (numFields.find(f=>f.slot===3)?.title || 'Number 3') }
  ...
  res.json({
    counts: { items: count, likes: likeCount },
    numbers: [
      { title: titles[1], min: nums?.num1_min ?? null, max: nums?.num1_max ?? null, avg: nums?.num1_avg ?? null, median: nums?.num1_med ?? null },
      { title: titles[2], min: nums?.num2_min ?? null, max: nums?.num2_max ?? null, avg: nums?.num2_avg ?? null, median: nums?.num2_med ?? null },
      { title: titles[3], min: nums?.num3_min ?? null, max: nums?.num3_max ?? null, avg: nums?.num3_avg ?? null, median: nums?.num3_med ?? null },
    ],
    topText,
    timeline,
    contributors,
