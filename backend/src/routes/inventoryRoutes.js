...
// List all / mine / canWrite
router.get('/', async (req, res) => {
  const { mine, canWrite: cw } = req.query
...
// Get one + fields + elements + last 100 items
router.get('/:id', async (req, res) => {
  ...
  const items = await prisma.item.findMany({
    where: { inventoryId: inv.id }, orderBy: { createdAt: 'desc' }, take: 100
  })
  res.json({
    inventory: inv,
...
router.put('/:id/items/:itemId', requireAuth, async (req, res) => {
  ...
  const allowed = ['customId', ...[1,2,3].flatMap(n=>[
    `text${n}`, `mtext${n}`, `num${n}`, `link${n}`, `bool${n}`
  ])]
  Object.keys(data).forEach(k => { if (!allowed.includes(k)) delete data[k] })

  const item = await prisma.item.update({ where: { id: req.params.itemId }, data })
  res.json(item)
})
...
// Comments & stats stay as you had:
