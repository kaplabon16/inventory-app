...
router.get('/google/callback',
  ...
  async (req, res) => { setCookieToken(res, req.user); res.redirect(`${frontendBase}/`) }
)
...
router.post('/register', async (req, res) => {
  try {
    ...
    res.json(safe)
  } catch (e) { console.error('REGISTER_ERR', e); res.status(500).json({ error:'SERVER_ERROR' }) }
})
...
router.post('/login', async (req, res) => {
  try {
    ...
    setCookieToken(res, safe)
    res.json(safe)
  } catch (e) { console.error('LOGIN_ERR', e); res.status(500).json({ error:'SERVER_ERROR' }) }
})
...
router.post('/set-password', async (req, res) => {
  ...
  } catch (e) { console.error('SET_PW_ERR', e); res.status(401).json({ error:'UNAUTHORIZED' }) }
})

// --- Session helpers
router.get('/me', async (req, res) => {
  try {
    const raw = bearerOrCookie(req); if (!raw) return res.json(null)
    const { id } = jwt.verify(raw, process.env.JWT_SECRET)
    const user = await prisma.user.findUnique({ where:{ id }, select:{ id:true, email:true, name:true, roles:true, blocked:true } })
    res.json(user || null)
  } catch { res.json(null) }
})
router.post('/logout', (req, res) => { res.clearCookie('token', { sameSite:'none', secure:true }); res.json({ ok:true }) })
