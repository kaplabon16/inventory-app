...
// GET /api/admin/users?q=&skip=&take=
...
router.patch('/users/:id/block', requireAdmin, async (req, res) => {
  try {
    const u = await prisma.user.update({ where: { id: req.params.id }, data: { blocked: true } })
    res.json({ ok: true, user: { id: u.id, blocked: u.blocked } })
  } catch { res.status(404).json({ error: 'User not found' }) }
})

router.patch('/users/:id/unblock', requireAdmin, async (req, res) => {
  try {
    const u = await prisma.user.update({ where: { id: req.params.id }, data: { blocked: false } })
    res.json({ ok: true, user: { id: u.id, blocked: u.blocked } })
  } catch { res.status(404).json({ error: 'User not found' }) }
})
...
router.patch('/users/:id/make-admin', requireAdmin, async (req, res) => {
  ...
  const roles = Array.from(new Set([...(u.roles || []).map(r => r.toLowerCase()), 'admin']))
  const updated = await prisma.user.update({ where: { id: u.id }, data: { roles } })
  res.json({ ok: true, user: { id: updated.id, roles: updated.roles } })
})

router.patch('/users/:id/remove-admin', requireAdmin, async (req, res) => {
  const u = await prisma.user.findUnique({ where: { id: req.params.id } })
  if (!u) return res.status(404).json({ error: 'User not found' })
  const roles = (u.roles || []).map(r => r.toLowerCase()).filter(r => r !== 'admin')
  const updated = await prisma.user.update({ where: { id: u.id }, data: { roles } })
  res.json({ ok: true, user: { id: updated.id, roles: updated.roles } })
})
